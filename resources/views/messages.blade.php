...
@if (session('error'))
    <div x-data="{ show: true }" x-show="show" @click.away="show = false"
        class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4 rounded-lg flex justify-between items-center"
        role="alert">
        <div class="flex items-center">
            <strong class="mr-2">Error!</strong> {{ session('error') }}
        </div>
        <button @click="show = false" class="btnCloseMessage text-red-500 hover:text-red-700" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
    </div>
@endif
