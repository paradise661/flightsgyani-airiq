        ...
        <div class="email-body">
            @if ($for == 'user')
                ...
                <p>You ticket has been issued for
                    ...
                    {{ $flightBooking->infant_count ? 'Infant: ' . $flightBooking->infant_count : '' }},
                    DEP: {{ $flightBooking->departure_date }}.
                </p>
            @else
                <p>Hello Admin,</p>
                <p>A customer has booked a flight. Please review the booking details.</p>
                <p>Ticket Details:
                    {{ $flightBooking->flight_type == 'O' ? 'One way' : 'Two way' }},
                    {{ $flightBooking->sector_from }}-{{ $flightBooking->sector_to }},
                    Adult: {{ $flightBooking->adult_count ?? 0 }}
            ...
            @if ($for == 'user')
                ...
                <a class="download-button"
                    href="{{ route('viewTicket', ['booking_code' => $flightBooking->booking_code, 'email' => $flightBooking->emergency_contact_email, 'type' => 'international']) }}">Download
                    Ticket</a>
            @else
                <p>Please click the link below to download ticket:</p>
                <a class="download-button"
                    href="{{ route('viewTicket', ['booking_code' => $flightBooking->booking_code, 'email' => $flightBooking->emergency_contact_email, 'type' => 'international']) }}">Download
                    Ticket</a>
                <p>Please check in admin panel for more details</p>
            @endif
        </div>
        <!-- Footer -->
