<div class="multi-city{{ $iteration }} mt-2 flex-col gap-2" style="display: {{ $i <= 1 ? 'flex' : 'none' }}">
    ...
    <div class="grid grid-cols-1 gap-2 w-full multi-city-group">
            ...
            <div class="border border-primary px-2 py-2 rounded-lg">
                <div class=" relative inline-flex w-full overflow-hidden">
                    <div class="flex flex-col items-start justify-center w-full" id="from-multi-drop{{ $iteration }}"
                        ...
                        {{-- <p class="text-xs text-black font-medium uppercase truncate overflow-hidden d-depairport{{(bool) $iteration ? '-'.$iteration : ''}}">
                        </p> --}}
                        <p
                            class="text-xs text-black font-medium uppercase truncate overflow-hidden d-depairport{{ (bool) $iteration ? '-' . $iteration : '' }} multi-depairport">
                        </p>
                        <input id="r-arrcity" type="text" hidden="">
                        @if ($i > 0)
                            <input id="rMultiFromAirport{{ $iteration }}" name="int_multi_from[]" type="text"
                                hidden>
                        @endif
                    </div>
                </div>
                <div
                    class="@if ($i > 0) open-multi-search-offcanvas @else open-international-search @endif stretched-link">
                </div>
            </div>
            <div
                ...
                {{-- <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none"
                    ...
                    <g id="SVGRepo_iconCarrier">
                        <path d="M8 3.5L8 16.5M8 3.5L3.5 7.83333M8 3.5L12.5 7.83333" stroke="#00a652" stroke-width="2"
        ...
        <div class="r-multi-depdate w-full">
            <div class="border border-primary px-2 py-2 rounded-lg flex flex-col items-start justify-center">
                ...
                <div class="relative">
                    <input
                        class=" @if ($i == 0) depdate @else multidepdate_mob @endif peer text-xs block$ px-0 py-0 w-full bg-transparent border-0 text-black font-semibold disabled:opacity-50 disabled:pointer-events-none tracking-wide focus:shadow-none focus:ring-0"
                        type="text" @if ($i > 0) name="int_multi_departure[]" @endif
...
<script>
    $(function() {
        $(".depdate").datepicker({
            ...
            onSelect: function(date) {
                ...
                $(".multidepdate_mob").datepicker("option", "minDate", date);
            },
        });

        $(".multidepdate_mob").datepicker({
            minDate: "0",
            format: "yyyy-mm-dd",
            startDate: "today",
            dateFormat: "yy-mm-dd",
    ...
    function updateMinDateMob() {
        var datepickers = $('.multidepdate_mob'); // Get all datepickers
