    ...
    <div class="col-md-12">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="flex-1">
                    <h3 class="text-lg font-semibold text-gray-800">Add New Staff</h3>
                </div>
            ...
            <!-- form start -->
            <form id="form" role="form" action="{{ route('v2.admin.users.store') }}" method="POST">
                @csrf
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700" for="phonenumber">Phone<span
                                    class="text-red-500">*</span></label>
                            <input class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm"
                                id="phonenumber" type="phonenumber" name="phonenumber" value="{{ old('phonenumber') }}">
                            @if ($errors->has('phonenumber'))
                                <span class="text-sm text-red-500">*{{ $errors->first('phonenumber') }}</span>
                            @endif
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700" for="password">Password<span
                                    class="text-red-500">*</span></label>
                            <input class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm"
                            ...
                            <select id="multiselect"
                                ...
                                @foreach ($roles as $role)
                                    <option value="{{ $role->name }}"
                                        @if(in_array($role->name, old('roles', []))) selected @endif>
                                        {{ $role->name ?? '' }}
                                    </option>
                                @endforeach
                            </select>
