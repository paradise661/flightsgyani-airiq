...
@section('content')
    ...
    <div class="col-md-12 relative w-full h-screen">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <form id="form" action="{{ route('v2.admin.sliders.update', $slider->id) }}" enctype="multipart/form-data"
                method="post" class="w-full mx-auto p-6">
                @csrf
                @method('PUT')

                @include('admin.v2.inc.slidercrop', ['image' => $slider->image])
                ...
                <div class="form-group">
                    <div class="flex items-center space-x-2">
                        <label for="status" id="statusLabel" class="text-sm font-medium text-gray-700">Active</label>
                        <input type="checkbox" id="status" name="status" {{ $slider->status ? 'checked' : '' }}
                            value="1" class="h-5 w-5 text-indigo-600 border-gray-300 rounded">
                    </div>
