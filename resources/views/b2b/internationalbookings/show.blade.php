    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-primary flex justify-between">
                ...
                <div class="box-header">
                    <a href="{{ route('b2b.agent.flight.bookings') }}"
                                        ...
                                        <tbody>
                                            @foreach ($booking->getTickets ?? [] as $ticket)
                                                <tr class="border-b">
                                                    ...
                                                    <td class="px-4 py-2 text-sm">
                                                        @if ($ticket->status)
                                                            <a href="{{ route('admin.flight.void.single.ticket', encrypt($ticket->id)) }}"
                                                                onclick="return confirm('Are you sure to void? You cannot revert it.')"
                                                                class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 text-xs font-semibold">
                                        ...
                                        @foreach (json_decode($booking->flights, true)['baggage'] ?? [] as $baggage)
                                            <tr class="border-b">
                                                ...
                                                <td class="px-6 py-4 text-sm text-gray-600">
                                                    {{ $baggage['description'] ? $baggage['description'] : '-' }}</td>
                    ...
                    <div class="tab-panel tab-content-box hidden" id="content8">
                        <div class="p-5">
                            ...
                            @if (!isset($booking->pnr_id))
                                <a href="{{ route('admin.flight.generate.pnr', $booking->booking_code) }}"
                                    class="mt-4 inline-block w-auto px-6 py-2 bg-red-600 text-white rounded-md text-sm font-medium text-center hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500">
                                    Generate PNR
                                </a>
                            @endif

                            @if (!isset($booking->ticket_details) && isset($booking->pnr_id) && !$booking->pnr_void)
                                <a href="{{ route('admin.flight.issue.ticket', $booking->booking_code) }}"
                            ...
                            @else
                                @isset($booking->ticket_details)
                                    <a href="{{ route('viewTicket', ['booking_code' => $booking->booking_code, 'email' => (json_decode($booking->contact_details, true)['email'] ?? ''), 'type' => 'international']) }}" target="_blank"
                                        class="mt-4 inline-block w-auto px-6 py-2 bg-green-600 text-white rounded-md text-sm font-medium text-center hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500">
                                        View Ticket
                                    </a>
                                    {{-- <a href="{{ route('show.ticket', $booking->booking_code) }}" target="_blank"
                                        class="mt-4 inline-block w-auto px-6 py-2 bg-green-600 text-white rounded-md text-sm font-medium text-center hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500">
                                        View Ticket
                                    </a> --}}
                                @endisset
                            ...
                            @if (help_canPnrBeVoid($booking->booking_code))
                                <a href="{{ route('admin.flight.void.pnr', $booking->booking_code) }}"
                                    class="mt-4 inline-block w-auto px-6 py-2 bg-red-600 text-white rounded-md text-sm font-medium text-center hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500">
                                    Void PNR
                                </a>
                            @endif

                            <a href="{{ route('admin.view.files', $booking->search_flight_id) }}" target="_blank"
                                class="mt-4 inline-block w-auto px-6 py-2 bg-gray-600 text-white rounded-md text-sm font-medium text-center hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500">
                                View Log Files
                            </a>
