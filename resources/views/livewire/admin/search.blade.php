        ...
        <div class="inline-block align-middle w-full bg-white rounded-lg shadow-md">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-neutral-900 dark:divide-neutral-700">
                        ...
                        @foreach ($searches as $key => $search)
                            <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700 transition duration-200 ease-in-out">
                                ...
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    {{ $search->id ?? '-' }}</td>
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    {{ isset($search->user_id) ? $search->searchedBy->name : '-' }}
                                </td>
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    @if (isset($search->sectors))
                                        {{ $search->departure . '->' . $search->destination . ',' }}
                                        @foreach (json_decode($search->sectors, true) ?? [] as $val)
                                            {{ $val['depart'] . '->' . $val['arrival'] . ',' }}
                                        @endforeach
                                    @else
                                        {{ $search->departure . '->' . $search->destination }}
                                    @endif
                                ...
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    {{ $search->return_date ? $search->return_date->toFormattedDateString() : '-' }}
                                </td>
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    {{ $search->adults . 'A/ ' . $search->childs . 'C/ ' . $search->infants . 'I' }}
                                </td>
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                ...
                                <td class="py-3 px-6 text-sm text-right flex justify-end">
                                    <a class="px-4 py-1 mr-2 text-white bg-green-600 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition duration-200 ease-in-out text-sm"
                                        href="{{ route('v2.admin.view.files', $search->id) }}">
                                        View
                                    </a>
                                    @can('delete searchlog')
                                        <a class="delete-record px-4 py-1 text-white bg-red-600 rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition duration-200 ease-in-out text-sm"
                                            href="{{ route('v2.admin.delete.flight.search', $search->id) }}">
                                            Delete
                                        </a>
                                    @endcan
                    ...
                    @else
                        <tr>
                            <td class="py-6 text-center text-gray-500 dark:text-neutral-400" colspan="9">
