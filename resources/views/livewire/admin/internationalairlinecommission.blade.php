                    ...
                    @if ($commissions->isNotEmpty())
                        @foreach ($commissions as $key => $commission)
                            <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700 transition duration-200 ease-in-out">
                                ...
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    {{ $commission->international_airline ?? '-' }}
                                </td>
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    {{ $commission->international_airline_code ?? '-' }}
                                </td>

                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    Adult: {{ $commission->commission ?? '0' }}, Child:
                                    {{ $commission->child_commission ?? '0' }}, Infant:
                                    {{ $commission->infant_commission ?? '0' }}
                                </td>
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    Adult: {{ $commission->agent_commission ?? '0' }}, Child:
                                    {{ $commission->agent_child_commission ?? '0' }}, Infant:
                                    {{ $commission->agent_infant_commission ?? '0' }}
                                ...
                                <td class="py-3 px-6 text-sm text-right flex justify-end">
                                    ...
                                    <a class="px-4 py-1 mr-2 text-white bg-green-600 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition duration-200 ease-in-out text-sm"
                                        href="{{ route('v2.admin.commissions.edit', $commission->id) }}">
                                        Edit
                                    </a>
                                    {{-- @endcan --}}
                                    {{-- @can('delete domesticairlinecommission') --}}
                                    <form action="{{ route('v2.admin.commissions.destroy', $commission->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button
