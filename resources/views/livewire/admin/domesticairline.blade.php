            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                <thead class="bg-gray-100 dark:bg-neutral-800">
                    <tr>
                        ...
                        <th class=" text-left text-sm font-medium text-gray-600 dark:text-neutral-200" scope="col">
                            Order
                        </th>
                        ...
                        @foreach ($airlines as $key => $airline)
                            <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700 transition duration-200 ease-in-out">
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    {{ $key + $airlines->firstItem() }}</td>

                                <td class="py-3 px-6">
                                    @if ($airline->image)
                                        <img src="{{ asset('uploads/domestic/airlines/' . $airline->image) ?: 'avatar.png' }}"
                                            alt="{{ $airline->name }}" width="60px">
                                    @endif
                                </td>
                                ...
                                <td class="py-3 px-6 text-sm text-gray-800 dark:text-neutral-200">
                                    {{ $airline->status == 1 ? 'Active' : 'Inactive' }}</td>
                                <td class="text-sm text-gray-800 dark:text-neutral-200">
                                    {{ $airline->order ?? '-' }}
                                </td>

                                <td class="py-3 px-6 text-sm text-right flex justify-end">
                                    @can('edit domesticairline')
                                        <a class="px-4 py-1 mr-2 text-white bg-green-600 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 transition duration-200 ease-in-out text-sm"
                                            href="{{ route('v2.admin.domestic.airlines.edit', $airline->id) }}">
                                            Edit
                                        </a>
                                    @endcan
                                    @can('delete domesticairline')
                                        <form action="{{ route('v2.admin.domestic.airlines.destroy', $airline->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                ...
                                <p class="mt-2 text-sm">There are no records to display at the moment. Please check
                                    ...
                                    <span>
                                        <a class="text-blue-600 hover:text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition duration-200 ease-in-out"
                                            href="{{ route('v2.admin.domestic.airlines.create') }}">
                                            add new entries.
                                        </a>
                                    </span>
                                </p>
