...
class BargainFinderMaxAdRQ extends SabreBasic
{
    ...
    public function generateBody()
    {
        ...
        $travel_preferences_array['VendorPref'] = [
            '_attributes' => [
                'Code' => "BA",
        ...
        if (isset($search->sectors)) {
            if ($search->nationality == 'NP' && $search->departure == 'KTM') {
    ...
    public function formatResponse($doc)
    {
        ...
        foreach ($iteneraies as $itinerary) {
            ...
            foreach ($destination_options as $option) {
                $totalFlightTime = $option->getAttribute('ElapsedTime');
                $flight = [];
                $f_count = 0;
                ...
                foreach ($flight_segments as $segment) {
                    ...
                    array_push($flight, [
                        ...
                        'valadatingairline' => $validating_carrier
                    ]);
                }

                array_push($flights[$rph]['flight'], [
                    'sectors' => $flight,
    ...
    public function addDetail($flights)
    {
        ...
        foreach ($flights as $flight) {
            ...
            foreach ($flight['flight'] as $tempFlight) {
                $f = $tempFlight['sectors'];
                $s = 100;
                $totaltime = 0;
                $f_count = count($f);
                ...
                array_push($flight['detail'], [
                    ...
                    'airline' => $airline,
                    'totaltime' => $this->generateFlightTime($tempFlight['time']),
