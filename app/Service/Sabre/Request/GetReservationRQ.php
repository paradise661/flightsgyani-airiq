...
class GetReservationRQ extends SabreBasic
{
    public function doRequest($action, $pnr)
    {
//        dd($pnr);
        ...
        if (is_dir('../storage/app/public/international/' . session()->get('flight_search'))) {
            $checkfile = '../storage/app/public/international/' . session()->get('flight_search') . '/GetReservationRQ.txt';
            if (file_exists($checkfile)) {
                $file = '../storage/app/public/international/' . session()->get('flight_search') . '/GetReservationRQ' . time() . '.txt';
            } else {
                $file = $checkfile;
            }
            ...
            $file = '../storage/app/public/international/' . session()->get('flight_search') . '/GetReservationRQ.txt';
            file_put_contents($file, $xmlstr);
        }
        $response = $this->doSoapRequest($xmlstr);
        $checkfile = '../storage/app/public/international/' . session()->get('flight_search') . '/GetReservationRS.txt';
        if (file_exists($checkfile)) {
            $file = '../storage/app/public/international/' . session()->get('flight_search') . '/GetReservationRS' . time() . '.txt';
        } else {
            $file = $checkfile;
        }
...
//            $meal = $details->getElementsByTagName('Meal')->item(0)->getAttribute('Code')?? NULL;
          /*  if ($details->getElementsByTagName('Meal')!== null) {
...
//                'meal' => $meal,
        ...
        $pricequote = $doc->getElementsByTagName('PriceQuote')->item(0);
        $pricequoteinfo = $pricequote->getElementsByTagName('PriceQuoteInfo')->item(0);
        $detailspricequote = $pricequoteinfo->getElementsByTagName('Details');
        foreach ($detailspricequote as $detailBaggage) {
            $pricequotesegmentInfo = $detailBaggage->getElementsByTagName('SegmentInfo')->item(0);
            array_push($baggages, [
                'passengerType' => $detailBaggage->getAttribute('passengerType'),
                'allowance' => $pricequotesegmentInfo->getElementsByTagName('Baggage')->item(0)->getAttribute('allowance'),
                'type' => $pricequotesegmentInfo->getElementsByTagName('Baggage')->item(0)->getAttribute('type'),
            ]);
        }

        $formatted_response['flights'] = $flights_details;
        $formatted_response['tickets'] = $tickets;
        $formatted_response['baggages'] = $baggages;
